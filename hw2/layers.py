...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        # relu' = alpha if x < alpha * x else 1, if 0<= alpha < 1, d= alpha when x is negative.
        dx = dout.clone()
        dx[x < 0] *= self.alpha
        # ========================
        return dx
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        loss = torch.mean(-x[torch.arange(N), y] + torch.log(torch.sum(torch.exp(x), dim=1)))
        # ========================
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        # not using dout because it is set to 1.
...
class MLP(Layer):
    ...
    def __init__(
        ...
        layers.append(Linear(hidden_features[-1] if hidden_features else in_features, num_classes))
        # ========================
